...
const deepMap = (obj, fn) => {
  if (Array.isArray(obj)) return obj.map((item) => deepMap(item, fn));
  if (obj !== null && typeof obj === 'object') {
    return Object.fromEntries(
      Object.entries(obj).map(([key, value]) => [key, deepMap(value, fn)]),
    );
  }
  return fn(obj);
};
...
const getAwsContext = ({ config, variables }) =>
  Promise.all([
    getAccountId(),
    getRegion(),
    getOutputs({
  ...
  ]).then(([accountId, region, cfOutputs]) => ({
    ...cfOutputs,
    accountId,
    region,
  }));
    ...
    Promise.resolve()
      ...
      .then((config) =>
        Promise.all([
          ...
          get(config, 'provider.name') === 'key-vault'
            ? Promise.resolve({})
            : getAwsContext({ config, variables }),
        ]),
      )
      .then(([validatedConfig, providerContext]) => {
        const mergedVariables = { ...variables, ...providerContext };
