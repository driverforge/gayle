...
describe('getSettings', () => {
  it('should throw an error when the provider is invalid', () => {
    ...
    return expect(getSettings()).rejects.toEqual(
      new Error(`Invalid provider 's3'!! Only ssm,key-vault are supported.`)
    );
  });
  ...
  it('should throw an error if the file type is not supported', () => {
    ...
    return expect(getSettings()).rejects.toEqual(
      new Error(
        `Could not find gayle.yml in the following directory - ${process.cwd()}`
      )
    );
  });
  ...
  it('should interpolate the yaml file with the variables provided', () => {
    const getSettings = makeGetSettings({
      settingsFilePath: path.resolve(process.cwd(), './mocks/ssm-provider.yml'),
      variables: {
        stage: 'test'
      }
    });
  ...
  it('should load key-vault settings without AWS calls', () => {
    const getSettings = makeGetSettings({
      settingsFilePath: path.resolve(
        process.cwd(),
        './mocks/key-vault-provider.yml'
      ),
      variables: {
        stage: 'production'
      }
    ...
    return getSettings().then(settings => {
      expect(settings).toEqual({
        config: {
          ...
          path: 'graph',
          required: {
            DB_TABLE: 'some database table name for production'
          }
        },
        configParameters: ['graph/DB_NAME', 'graph/DB_HOST', 'graph/DB_TABLE'],
        provider: {
          name: 'key-vault',
          vault: 'driverforge-production'
        },
        secret: {
          path: 'graph',
          required: {
        ...
        secretParameters: ['graph/DB_PASSWORD'],
        service: 'oprah-service'
      });
    });
  });
