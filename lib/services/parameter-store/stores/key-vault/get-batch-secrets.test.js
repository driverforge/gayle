...
describe('getBatchSecrets', () => {
  ...
  it('should fetch secrets from key vault', () => {
    expect(mockGetSecret).toHaveBeenCalledWith('graph--DB-NAME');
    expect(mockGetSecret).toHaveBeenCalledWith('graph--DB-HOST');
    expect(mockGetSecret).toHaveBeenCalledWith('graph--MISSING');
  });
