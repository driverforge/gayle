...
const getBatchParameters = ({ parameterNames }) => {
  const size = 10;
  const chunks = [...Array(Math.ceil(parameterNames.length / size))].map(
    (_, i) => parameterNames.slice(i * size, i * size + size),
  );

  const chunksOfRequests = chunks.map(
    (chunkOfTen) => () => getParametersFromSSM({ parameterNames: chunkOfTen }),
  );
