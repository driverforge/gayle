...
describe('cleanup', () => {
  ...
  it('should delete unused parameters', async () => {
    ...
    expect(mockLog).toHaveBeenCalledWith(
      expect.stringContaining('Deleting unused parameter: /test/config/THREE'),
    );
  });

  it('should refuse to clean up when no parameters are declared', async () => {
    ...
    await expect(oprah.cleanUp()).rejects.toThrow(/Cleanup refused/);
    expect(mockGetAllParameters).not.toHaveBeenCalled();
    expect(mockDeleteParameters).not.toHaveBeenCalled();
  });

  it('should refuse to clean up when declared parameters are missing entirely', async () => {
    mockGetSettings.mockResolvedValue({
      config: { path: '/test/config' },
